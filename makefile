...
migrate-local-down-all:
	migrate -path migrations -database "$(POSTGRESQL_URL)" down
	
migrate-local-reset:
	migrate -path migrations -database "$(POSTGRESQL_URL)" drop -f


migrate-local-force-zero:
	migrate -path migrations -database "$(POSTGRESQL_URL)" force 0
	@echo "Migration force done"
